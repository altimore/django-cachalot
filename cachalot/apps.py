...
from django import __version__ as django__version__
...
from .settings import (SUPPORTED_CACHE_BACKENDS, SUPPORTED_DATABASE_ENGINES,
                       SUPPORTED_ONLY, cachalot_settings)


@register(Tags.compatibility)
def check_django_version(app_configs, **kwargs):
    if not (1, 11) <= django_version:
        return [
            Error(
...
def check_cache_compatibility(app_configs, **kwargs):
    ...
    if cache_backend not in SUPPORTED_CACHE_BACKENDS:
        return [
            Warning(
                "Cache backend %r is not supported by django-cachalot." % cache_backend,
